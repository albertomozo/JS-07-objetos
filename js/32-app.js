...
function actualizarInterfaz() {
    ...
    //for (let i = 1; i <= plazas)
    const porcentaje = (curso.matriculas) * 100 / curso.alumnosMaximo;
    document.getElementById('info').innerHTML = `
    <progress id="alumnos" max="100" value="${porcentaje}">${curso.matriculas} /${curso.alumnosMaximo}</progress>${curso.matriculas} /${curso.alumnosMaximo}`;
...
document.getElementById('titulo').innerText = '32  -  barra progreso';
